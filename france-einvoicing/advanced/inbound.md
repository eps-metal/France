...
- Current version is v3.1; v3.0 introduced the PA (Plateforme Agréée) terminology that replaced earlier draft language
...
- 50+ mandatory rules cover: mandatory field presence, SIREN format correctness (9 digits, no letters), VAT calculation accuracy (amount must match rate × taxable base), and line-item totals matching the invoice header total
...
| Approuvée | Acknowledged | Recipient system confirms receipt | Recipient (via PA) | None — invoice in process |
...
- **Happy path**: Déposée → Reçue → Approuvée → Encaissée — invoice submitted, delivered, acknowledged, and payment reported to DGFiP
...
| REST API | HTTP, sync/event | Modern integrations; event-driven architectures | DGFiP spec v3.1 defines exact endpoints and authentication |

### Peppol Interoperability

- DGFiP is the French Peppol authority — the designated national access point connecting France to the European Peppol network
...
- French tax law requires invoices to be retained for 10 years from the invoice date; this applies to both received and issued invoices
